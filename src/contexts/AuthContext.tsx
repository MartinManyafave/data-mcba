...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (_event, session) => {
        ...
        if (session?.user) {
          sessionStorage.setItem("data_mcba_active_session", "true");
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (!error) sessionStorage.setItem("data_mcba_active_session", "true");
  ...
  const signOut = async () => {
    sessionStorage.removeItem("data_mcba_active_session");
    await supabase.auth.signOut();
  };

  const resetPassword = async (email: string) => {
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `https://data-mcba-eqha.vercel.app/login`,
    });
    return { error: error as Error | null };
  };
